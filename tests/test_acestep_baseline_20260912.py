...
class ACEBaseLine20260912Tests(unittest.TestCase):
    ...
    def test_vram_policy_preserves_kv_floor_and_hrb_authority(self):
        self.assertTrue(vram_reserve_valid(
            model_aware=True,
            duration_aware=True,
            batch_aware=True,
        ...
        self.assertFalse(vram_reserve_valid(
            model_aware=True,
            duration_aware=True,
            batch_aware=True,
    ...
    def test_lossless_master_and_torchcodec_fail_closed(self):
        self.assertTrue(export_path_valid(
            fmt="flac",
            torchcodec_required=False,
            fallback_verified=True,
            linux_dependency_smoke=True,
        ))
        self.assertFalse(export_path_valid(
            fmt="mp3",
            torchcodec_required=False,
            fallback_verified=True,
            linux_dependency_smoke=True,
        ))
        self.assertTrue(torchcodec_enablement_valid(
            enabled=True,
            torch_abi_match=True,
            torchaudio_abi_match=True,
            codec_import_smoke=True,
        ))
        self.assertFalse(torchcodec_enablement_valid(
            enabled=True,
