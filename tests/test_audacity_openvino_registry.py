...
def test_audacity_profile_is_non_authoritative_projection():
    ...
    assert p['new_capability'] is False and p['new_architectural_authority'] is False
    assert p['subprofile_of'] == 'FA3-AUDIO-001'
    assert 'FA3-ACCEL-GUARD-001' in p['dependencies']

def test_base_editor_has_no_accelerator_requirement():
    p = load('canonical/providers/FA3-PROVIDER-AUDACITY-001.json')
    assert p['requirement'] == 'SHOULD'
    assert p['hardware_semantics'].startswith('BASE_EDITOR_HAS_NO_ACCELERATOR_REQUIREMENT')
...
def test_openvino_cannot_capture_authority():
    ...
    assert 'Host Resource Broker' in constraints and 'FA3-ACCEL-GUARD-001' in constraints
...
def test_contract_requires_provenance_and_admission_receipt():
    ...
    assert {'hrb_lease','accelerator_guard_reconciliation'} <= set(admission['accelerator_fields_required_only_when_accelerator_requested'])
