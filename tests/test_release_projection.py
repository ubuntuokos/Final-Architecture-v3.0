...
class ReleaseProjectionGateTests(unittest.TestCase):
    ...
    def test_openyak_projection_reconciliation_fails_closed(self):
        td, dst, facts = self._copy_repo()
        try:
            path = dst / PROJECTION_PATH
            obj = json.loads(path.read_text(encoding="utf-8"))
            obj["openyak_reconciliation"]["provider_id"] = "INVALID"
            path.write_text(json.dumps(obj, indent=2) + "\n", encoding="utf-8")
            report = self._gate_copy(dst, facts)
            self.assertEqual("FAIL", report["result"])
            self.assertTrue(any(x["code"] == "FA3-RELEASE-PROJECTION-042" for x in report["findings"]))
        finally:
            td.cleanup()

    def test_openyak_reference_cannot_claim_current_host_runtime(self):
        td, dst, facts = self._copy_repo()
        try:
            path = dst / PROJECTION_PATH
            obj = json.loads(path.read_text(encoding="utf-8"))
            obj["openyak_reconciliation"]["runtime_activation_status"] = "ADMITTED"
            obj["openyak_reconciliation"]["current_host_runtime_evidence"] = "CURRENT_HOST_PRODUCTION_E2E_PASS"
            path.write_text(json.dumps(obj, indent=2) + "\n", encoding="utf-8")
            report = self._gate_copy(dst, facts)
            self.assertEqual("FAIL", report["result"])
            self.assertTrue(any(x["code"] == "FA3-RELEASE-PROJECTION-042" for x in report["findings"]))
    ...
    def test_lynxhub_projection_reconciliation_fails_closed(self):
        td, dst, facts = self._copy_repo()
        try:
            path = dst / PROJECTION_PATH
            obj = json.loads(path.read_text(encoding="utf-8"))
            obj["lynxhub_reconciliation"]["provider_id"] = "INVALID"
            path.write_text(json.dumps(obj, indent=2) + "\n", encoding="utf-8")
            report = self._gate_copy(dst, facts)
            self.assertEqual("FAIL", report["result"])
            self.assertTrue(any(x["code"] == "FA3-RELEASE-PROJECTION-043" for x in report["findings"]))
        finally:
            td.cleanup()

    def test_lynxhub_reference_cannot_claim_current_host_runtime(self):
        td, dst, facts = self._copy_repo()
        try:
            path = dst / PROJECTION_PATH
            obj = json.loads(path.read_text(encoding="utf-8"))
            obj["lynxhub_reconciliation"]["runtime_activation_status"] = "ADMITTED"
            obj["lynxhub_reconciliation"]["current_host_runtime_evidence"] = "CURRENT_HOST_PRODUCTION_E2E_PASS"
            path.write_text(json.dumps(obj, indent=2) + "\n", encoding="utf-8")
            report = self._gate_copy(dst, facts)
            self.assertEqual("FAIL", report["result"])
            self.assertTrue(any(x["code"] == "FA3-RELEASE-PROJECTION-043" for x in report["findings"]))
    ...
    def test_tencentdb_agent_memory_projection_reconciliation_fails_closed(self):
        td, dst, facts = self._copy_repo()
        try:
            path=dst/PROJECTION_PATH;obj=json.loads(path.read_text(encoding="utf-8"));obj["tencentdb_agent_memory_reconciliation"]["provider_id"]="INVALID";path.write_text(json.dumps(obj,indent=2)+"\n",encoding="utf-8");report=self._gate_copy(dst,facts);self.assertEqual("FAIL",report["result"]);self.assertTrue(any(x["code"]=="FA3-RELEASE-PROJECTION-034" for x in report["findings"]))
        finally: td.cleanup()

    def test_tencentdb_agent_memory_reference_cannot_claim_runtime(self):
        td, dst, facts = self._copy_repo()
        try:
            path=dst/PROJECTION_PATH;obj=json.loads(path.read_text(encoding="utf-8"));obj["tencentdb_agent_memory_reconciliation"]["current_host_runtime_evidence"]="CURRENT_HOST_PRODUCTION_E2E_PASS";obj["tencentdb_agent_memory_reconciliation"]["production_provider_admission"]=True;path.write_text(json.dumps(obj,indent=2)+"\n",encoding="utf-8");report=self._gate_copy(dst,facts);self.assertEqual("FAIL",report["result"]);self.assertTrue(any(x["code"]=="FA3-RELEASE-PROJECTION-034" for x in report["findings"]))
        finally: td.cleanup()


    def test_loop_engineering_projection_reconciliation_fails_closed(self):
        td, dst, facts = self._copy_repo()
        try:
            path = dst / PROJECTION_PATH
            obj = json.loads(path.read_text(encoding="utf-8"))
            obj["loop_engineering_reconciliation"]["contract_id"] = "INVALID"
            path.write_text(json.dumps(obj, indent=2) + "\n", encoding="utf-8")
            report = self._gate_copy(dst, facts)
            self.assertEqual("FAIL", report["result"])
            self.assertTrue(
                any(
                    item["code"] == "FA3-RELEASE-PROJECTION-036"
                    for item in report["findings"]
                )
            )
        finally:
    ...
    def test_loop_engineering_cap028_binding_fails_closed(self):
        td, dst, facts = self._copy_repo()
        try:
            path = dst / "evidence/evidence-registry.json"
            obj = json.loads(path.read_text(encoding="utf-8"))
            cap028 = next(item for item in obj["records"] if item["subject_id"] == "CAP-028")
            cap028["source_decision_ids"].remove("FA3-DEC-LOOP-ENGINEERING-2026-09-03")
            path.write_text(json.dumps(obj, indent=2) + "\n", encoding="utf-8")
            report = self._gate_copy(dst, facts)
            self.assertEqual("FAIL", report["result"])
            self.assertTrue(
