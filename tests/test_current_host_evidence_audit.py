...
class CurrentHostEvidenceAuditTests(unittest.TestCase):
    def _root(self):
        ...
        shutil.copy(
            ROOT / "evidence/reference/hrb-cuda-current-host-2026-08-28.json",
            root / "evidence/reference/hrb-cuda-current-host-2026-08-28.json",
        )
        return td, root
    ...
    def test_fabricated_registry_pass_without_receipt_fails_integrity(self):
        ...
        try:
            ...
            reg["records"][0]["status"] = "PASS"
            p.write_text(json.dumps(reg))
            r = audit(root)
            self.assertEqual(r["audit_integrity"], "FAIL")
            self.assertIn("CAP-001", r["blocking_findings"][0]["capability_ids"])
        finally:
            td.cleanup()
    ...
    def test_valid_receipt_can_be_reconciled(self):
        td, root = self._root()
        try:
            reg = json.loads((root / "evidence/evidence-registry.json").read_text())
            rec = reg["records"][0]
            ...
            r = audit(root, apply_reconciliation=True)
            self.assertIn("CAP-001", r["reconciliation_applied"])
            self.assertEqual(
                json.loads((root / "evidence/evidence-registry.json").read_text())["records"][0]["status"],
                "PASS",
            )
        finally:
            td.cleanup()
    ...
    def test_test_id_mismatch_is_rejected(self):
        ...
        try:
            reg = json.loads((root / "evidence/evidence-registry.json").read_text())
            receipt = self._valid_receipt(root, reg["records"][0])
            receipt["tests"]["positive"]["id"] = "AT-WRONG"
            (root / "evidence/receipts/capabilities/CAP-001.json").write_text(json.dumps(receipt))
            row = audit(root)["capabilities"][0]
            self.assertFalse(row["qualified_current_host_receipt"])
    ...
    def test_declared_digest_must_match_real_file(self):
        ...
        try:
            reg = json.loads((root / "evidence/evidence-registry.json").read_text())
            receipt = self._valid_receipt(root, reg["records"][0])
            receipt["evidence_artifacts"][0]["sha256"] = "b" * 64
            (root / "evidence/receipts/capabilities/CAP-001.json").write_text(json.dumps(receipt))
            row = audit(root)["capabilities"][0]
            self.assertFalse(row["qualified_current_host_receipt"])
