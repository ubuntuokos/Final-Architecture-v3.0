...
class MotionVideoAdapterTests(unittest.TestCase):
    ...
    def test_real_command_transport_executes_pinned_binary(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            sel=self.base_selection()
            sel["provider_manifest_path"]=str(manifest.resolve())
            sel["provider_manifest_sha256"]=hashlib.sha256(manifest.read_bytes()).hexdigest()
            selection.write_text(json.dumps(sel),encoding="utf-8")
