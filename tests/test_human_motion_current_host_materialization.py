...
class HumanMotionCurrentHostMaterializationTests(unittest.TestCase):
    def test_current_host_descriptor_is_fail_closed_and_non_authoritative(self):
        ...
        self.assertEqual("PENDING_REAL_CURRENT_HOST_EXECUTION", c["status"]); self.assertFalse(c["production_admitted"]); self.assertEqual(module_active_capability_count(__file__), c["capability_count_after"]); self.assertEqual(0, c["new_architectural_authorities"]); self.assertTrue(c["fail_closed"])
    ...
    def test_decision_preserves_architecture(self):
        d = load("canonical/decisions/FA3-DEC-HUMAN-MOTION-CURRENT-HOST-2026-09-12.json"); self.assertEqual(module_active_capability_count(__file__), d["baseline_effect"]["capability_count_after"]); self.assertEqual(0, d["baseline_effect"]["new_architectural_authorities"]); self.assertFalse(d["security"]["runtime_network_fetch"]); self.assertFalse(d["security"]["automatic_model_download"])
        h = load("canonical/decisions/FA3-DEC-HUMAN-MOTION-HARDWARE-CONFORMANCE-2026-09-12.json"); self.assertEqual(module_active_capability_count(__file__), h["baseline_effect"]["capability_count_after"]); self.assertEqual(0, h["baseline_effect"]["new_architectural_authorities"]); self.assertEqual("FA3-AUTH-HOST-RESOURCE-BROKER-001", h["placement_authority_id"])
