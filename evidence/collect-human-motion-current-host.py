...
def live_gpus() -> list[dict[str, Any]]:
    p = must(["nvidia-smi", "--query-gpu=index,uuid,pci.bus_id,name,memory.total", "--format=csv,noheader,nounits"], timeout=30)
    rows = []
...
def validate_hrb(value: dict[str, Any], gpus: list[dict[str, Any]]) -> dict[str, Any]:
    if not (value.get("schema") == "fa3.hrb-placement-receipt.v1" and value.get("authority_id") == "FA3-AUTH-HOST-RESOURCE-BROKER-001" and value.get("status") in {"ADMITTED", "ACTIVE"} and value.get("lease_id") and value.get("device_uuid") and value.get("pci_bdf") and value.get("placement_source") == "LIVE_TOPOLOGY" and value.get("static_runtime_ordinal_as_identity") is False):
        raise RuntimeError("invalid HRB placement receipt")
    expires = parse_time(value.get("expires_at"))
    if expires <= datetime.now(timezone.utc): raise RuntimeError("HRB lease expired")
    ...
    if len(matches) != 1: raise RuntimeError("HRB UUID/BDF does not resolve to exactly one live GPU")
    return {"valid": True, "lease_id_sha256": hashlib.sha256(str(value["lease_id"]).encode("utf-8")).hexdigest(), "device_uuid": uuid, "pci_bdf": bdf, "expires_at": expires.isoformat().replace("+00:00", "Z"), "workload_class": value.get("workload_class"), "placement_source": "LIVE_TOPOLOGY"}


def require_schema(value: dict[str, Any], schema: str, label: str) -> None:
    if value.get("schema") != schema: raise RuntimeError(f"{label}: schema must be {schema}")


def resolved_path(value: Any, label: str) -> Path:
    if not isinstance(value, str) or not value: raise RuntimeError(f"{label}: path missing")
    p = Path(os.path.expandvars(os.path.expanduser(value))).resolve()
    if not p.exists(): raise RuntimeError(f"{label}: path does not exist")
    return p
...
def verify_runtime_assets(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if not (value.get("admitted") is True and value.get("all_third_party_assets_admitted") is True): raise RuntimeError("runtime assets are not admitted")
    ...
    for field, actual in observed.items():
        if value.get(field) != actual: raise RuntimeError(f"runtime asset hash mismatch: {field}")
...
def verify_smplx(value: dict[str, Any], soma_data_root: Path) -> dict[str, Any]:
    ...
    if root != soma_data_root: raise RuntimeError("SMPL-X and SOMA-X converter data roots must be identical")
    digest = digest_tree(root)
    if not (value.get("accepted") is True and value.get("model_assets_present") is True and value.get("asset_sha256") == digest): raise RuntimeError("SMPL-X license/asset receipt mismatch")
...
def ffprobe_video(path: Path) -> dict[str, Any]:
    ...
    if not streams: raise RuntimeError("input video has no video stream")
    ...
    if frames < 8 or duration <= 0 or fps <= 0: raise RuntimeError("validation video is too short or has invalid timing")
...
def verify_video(path: Path, admission: dict[str, Any]) -> dict[str, Any]:
    ...
    if not (admission.get("artifact_sha256") == digest and admission.get("synthetic") is False and admission.get("rights_or_consent_confirmed") is True): raise RuntimeError("video rights/consent/non-synthetic admission mismatch")
...
def offline_env(gpu_uuid: str) -> dict[str, str]:
    ...
    for key in ("HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "all_proxy", "HF_TOKEN", "HUGGING_FACE_HUB_TOKEN"): env.pop(key, None)
    return env
...
def inspect_smplx(soma_py: Path, npz: Path, env: dict[str, str]) -> dict[str, Any]:
    ...
    if not result.get("finite"): raise RuntimeError(f"SMPL-X roundtrip contains invalid arrays: {result}")
    return result
...
def dcc_smoke(usd: Path, env: dict[str, str], runtime_dir: Path) -> dict[str, Any]:
    ...
    for app, exe in candidates:
        ...
        if p.returncode == 0 and count > 0: return {"status": "PASS", "application": app, "imported_object_count": count, "log_sha256": hashlib.sha256(log.encode("utf-8", errors="replace")).hexdigest()}
...
def main() -> int:
    ...
    try:
        if platform.system() != "Linux" or platform.machine().lower() not in {"x86_64", "amd64"}: raise RuntimeError("Linux x86_64 current host required")
        if hasattr(os, "geteuid") and os.geteuid() == 0: raise RuntimeError("current-host E2E must not run as root")
        for cmd in ("git", "ffprobe", "nvidia-smi", "bwrap"):
            if not shutil.which(cmd): raise RuntimeError(f"required executable missing: {cmd}")
        ...
        for path, label in ((checkpoint, "GEM-X checkpoint"), (video, "input video")):
            if not path.is_file(): raise RuntimeError(f"{label} missing")
        gem_py = gem_root / ".venv/bin/python"; soma_py = soma_root / ".venv/bin/python"
        if not gem_py.is_file() or not soma_py.is_file(): raise RuntimeError("pre-provisioned GEM-X and SOMA-X venv Python executables are required")
        ...
        if not (cuda.get("available") is True and cuda.get("count") == 1): raise RuntimeError(f"HRB-scoped CUDA probe failed: {cuda}")
        ...
        if re.search(r"\bdownload(?:ing|ed)?\b.*huggingface|\[checkpoint\].*download", gem_log, re.I): raise RuntimeError("GEM-X attempted runtime model/network download")
        if re.search(r"fall(?:ing)? back to cpu|using cpu execution provider", gem_log, re.I): raise RuntimeError("silent CPU fallback observed")
        ...
        if not (len(hpe) == len(vitpose) == len(camera) == 1): raise RuntimeError(f"unexpected GEM-X output cardinality hpe={len(hpe)} vitpose={len(vitpose)} camera={len(camera)}")
        ...
        if not metrics.get("motion", {}).get("dynamic_camera_trajectory_present"): raise RuntimeError("dynamic-camera validation failed; static/identity trajectory detected")
        ...
        if re.search(r"fall(?:ing)? back to cpu", conversion_log, re.I): raise RuntimeError("SOMA-X conversion fell back to CPU")
        ...
        if not usds: raise RuntimeError("SOMA-X pose converter did not export USD")
        ...
        receipt.update({"status": "PASS", "evidence_level": EVIDENCE_LEVEL, "completed_at": now(), "sources": sources, "checkpoint": {"sha256": checkpoint_sha, "admission": checkpoint_out}, "model_license": model_license_out, "soma_assets": {"admitted": True, "tree_sha256": assets["tree_sha256"]}, "smplx": smplx, "hrb": hrb, "gpu": {"uuid": gpu["uuid"], "pci_bdf": gpu["pci_bdf"], "name": gpu["name"], "memory_total_mib": gpu["memory_total_mib"]}, "input_video": video_info, "runtime_policy": {"network_fetch_disabled": True, "automatic_model_download_observed": False, "network_namespace": "BUBBLEWRAP_UNSHARE_NET"}, "execution": {"requested_backend": "cuda", "observed_cuda": True, "silent_cpu_fallback_observed": False, "gpu_uuid": gpu["uuid"], "cuda_device_count_inside_sandbox": cuda["count"], "gem_x_log_sha256": hashlib.sha256(gem_log.encode("utf-8", errors="replace")).hexdigest()}, "keypoints_2d": metrics["keypoints_2d"], "motion": metrics["motion"], "interchange": {"soma_npz_valid": True, "soma_npz_joint_count": metrics["soma_npz"]["joint_count"], "soma_npz_sha256": sha256_file(soma_npz), "smplx_roundtrip_pass": bool(smplx_stats.get("finite")), "smplx_npz_sha256": sha256_file(smplx_npz), "smplx_error_mean": smplx_stats.get("error_mean"), "smplx_error_max": smplx_stats.get("error_max"), "usd_export_pass": True, "usd_sha256": sha256_file(usd)}, "dcc_smoke": dcc})
        findings = validate_receipt(receipt)
        if findings: raise RuntimeError("final current-host receipt failed gate contract: " + json.dumps(findings))
