...
The provider-execution current-host path is additionally blocked while Secret Broker status is `CURRENT_HOST_REQUALIFICATION_REQUIRED`. The historical `2026-09-21` reference is provenance only and is explicitly denied as an active prerequisite. Provider execution may resume only after `evidence/reference/secret-broker-current-host-2026-09-24.json` is adopted and canonical Secret Broker conformance, gate, and enforcement all mark that reference active and production-admitted.
