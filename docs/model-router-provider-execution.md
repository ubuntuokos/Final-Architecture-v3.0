...
## Core closure versus provider admission

The provider-execution core is canonically closed by deterministic executable regressions and security/authority invariants in `canonical/FA3-MODEL-ROUTER-PROVIDER-EXECUTION-CORE-CLOSURE-001.json`. A provider-specific physical admission failure does not reopen that core.
