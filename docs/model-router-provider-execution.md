...
The generic harness reads both credential values only from `/dev/tty`, stores them temporarily under the Secret Broker as distinct `SecretReference` objects, restricts projection to a dedicated transient probe identity/unit, and preflights both credentials independently in two stages before model discovery: first identity authentication through `GET /v1/me`, then model-catalog access through `GET /v1/models`. A failed HTTP preflight reports only the credential label, HTTP status, and strictly sanitized provider `error.type` / `error.code`; provider `error.message`, response bodies, headers, and credential material are never logged. Only if both preflights pass does the harness perform the real before/after-rebind provider calls and write the secret-free live probe to `/run/fa3/model-router-provider-execution/current-host-source.json`. Temporary credential objects, policies, probe identity/runtime files, and any Secret Broker lifecycle started by the harness must be cleaned before collection/gating. `FA3 PROVIDER EXECUTION CURRENT-HOST: PASS` is withheld unless `provisioning_cleanup_pass` is proven. An unauthenticated provider endpoint cannot satisfy the gate.
