...
The custom-runner execution-plan compiler (`src/fa3_google_ax_custom_runner.py`) is now materialized. It preserves immutable Git semantics by performing HTTPS fetch-by-commit, detached checkout, and HEAD verification inside the runner plan; the Git host must be explicitly present in the default-deny network envelope. Skill and MCP workspace sources are not reinterpreted by the runner, so Skill Fabric and Central MCP Gateway remain the admission/mediation boundaries.\n\nThe fail-closed cluster-apply plan compiler (`src/fa3_google_ax_cluster_apply.py`) is now materialized as source/static infrastructure. It accepts only the pinned AX provider projection plus the custom-runner plan, requires a verified digest-bound runner-image evidence reference, a scope-bound Kubernetes context/Agent Substrate binding, and an explicit Security approval reference; raw kube credentials are rejected. It never emits AX `Model`, never changes HRB / Model Router / Central MCP Gateway authority, requires dry-run before server-side apply, and is not runtime-promotion evidence.\n\nThis remains **source/static materialization only**. No real digest-pinned runner image build evidence and no scope-bound cluster E2E are recorded. Runtime activation therefore remains `PENDING_RUNNER_IMAGE_BUILD_EVIDENCE_AND_SCOPE_BOUND_CLUSTER_E2E`.
