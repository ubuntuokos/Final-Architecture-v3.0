...
The encrypted credential remains delivered through `LoadCredentialEncrypted=` only to the mapper service. That service may therefore use its own filesystem namespace without affecting mount visibility. The broker retains its existing strong sandbox, receives no mount or mapper capability, and its `ExecStartPre` calls the read-only `assert-open` helper to verify that the path is an actual ext4 mount backed by the expected mapper, has `nodev,nosuid,noexec`, has broker ownership/mode, and contains the expected vault structure.
