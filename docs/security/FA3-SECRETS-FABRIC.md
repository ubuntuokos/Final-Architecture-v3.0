...
The host-visible filesystem mount is owned by the native systemd mount unit `run-fa3-machine\\x2dstate.mount`, corresponding to `/run/fa3/machine-state`. FA3 machine-secret mapper names are explicitly excluded from UDisks/file-manager discovery and automount; a raw machine vault appearing under a desktop path such as `/run/media/...` is a policy violation, not an alternate supported mount path. The mount unit requires the mapper service, mounts the mapper as ext4 with `rw,nodev,nosuid,noexec`, and is required by the unprivileged broker. Because PID 1 supervises the mount unit directly, the mount exists in the system manager's main mount namespace without requiring the credential-bearing service to disable normal filesystem namespace hardening or introspect `/proc/1/ns/mnt`.
...
New images persist `fa3-secret-broker:fa3-secret-broker` ownership and mode `0750` on the ext4 root at initialization, so mount ownership does not require privileged mutation during normal startup. Lifecycle shutdown is ordered broker → systemd mount unit → mapper service. Rollback explicitly stops all three layers and refuses to close a mapper while its mount remains active. After the mount disappears, mapper close tolerates only a short bounded device-release race: it retries close without force, while also refusing closure if `findmnt` still reports the mapper as mounted. Exhausted retries remain fail-closed and leave diagnostics instead of forcing device removal.

The physical current-host E2E uses a drop-in to point the canonical mount unit at its disposable mapper and proves from the host context that the mounted source resolves to that mapper. It also requires an `rw` mount and performs a real broker `put → get → revoke` sequence through the systemd-managed runtime before shutdown. It remains single-instance locked, detects stale E2E mappings, and requires both `systemd_vault_rw_mount_pass` and `systemd_broker_write_read_revoke_pass` before a PASS receipt can be emitted.
...
Repository/reference PASS alone does not promote production runtime. The 2026-09-21 Secret Broker current-host evidence at source commit `77288eb47946d93f4fb90100883151d7e06827bd` is retained as historical provenance only and no longer carries active runtime-promotion authority.

The 2026-09-24 requalification closed the physical gaps exposed by the production lifecycle probe: writable systemd-managed `rw,nodev,nosuid,noexec` vault operation, real non-root broker `put → get → revoke`, deterministic Unix-socket transport ownership, UDisks desktop-automount exclusion, bounded non-force mapper close, unlock-key rotation, and final CLOSED state were all proven on exact source commit `d18ba667f1e07c2f347d009713b865ce760b88ca`. The active durable evidence is `evidence/reference/secret-broker-current-host-2026-09-24.json`; Secret Broker core runtime status is therefore `CURRENT_HOST_ADMITTED` and `production_runtime_promoted=true` for `FA3_SECRET_BROKER_CORE_CURRENT_HOST_ONLY`.

This remains a component-scoped promotion only. Optional Podman or Infisical adapters retain their separate evidence requirements, CAP-003 may remain pending for other obligations, and `global_promotion_claim` remains false.
