...
The host-visible filesystem mount is owned by the native systemd mount unit `run-fa3-machine\\x2dstate.mount`, corresponding to `/run/fa3/machine-state`. The mount unit requires the mapper service, mounts the mapper as ext4 with `rw,nodev,nosuid,noexec`, and is required by the unprivileged broker. Because PID 1 supervises the mount unit directly, the mount exists in the system manager's main mount namespace without requiring the credential-bearing service to disable normal filesystem namespace hardening or introspect `/proc/1/ns/mnt`.
...
Repository/reference PASS alone does not promote production runtime. The 2026-09-21 Secret Broker current-host evidence at source commit `77288eb47946d93f4fb90100883151d7e06827bd` is retained as historical evidence, but it no longer carries active runtime-promotion authority.

A physical production lifecycle probe on 2026-09-24 exposed two requirements that the earlier systemd phase did not prove: the active vault was mounted read-only because the strict mapper-service sandbox did not explicitly permit write access to the backing image, and mapper close could encounter a short post-unmount busy-device race. The runtime status is therefore `CURRENT_HOST_REQUALIFICATION_REQUIRED`; `production_runtime_promoted=false` until fresh exact-source evidence proves an `rw,nodev,nosuid,noexec` mount, a real systemd-managed broker `put → get → revoke` sequence, bounded non-force mapper close, and final CLOSED state.

The superseded evidence remains at `evidence/reference/secret-broker-current-host-2026-09-21.json` for provenance. Fresh promotion evidence must be recorded separately and must not rewrite the historical receipt. Optional Podman or Infisical adapters still retain their separate evidence requirements, and `global_promotion_claim` remains false.
