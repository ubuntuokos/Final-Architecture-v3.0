...
AnimationAwareAppShell {
    ...
    // Window geometry follows the monitor, never the live UI-scale slider.
    ...
    component AskDialog: Dialog {
        ...
        contentItem: ColumnLayout {
            spacing: 10
            Label { Layout.fillWidth: true; text: shell.t("A szerep canonical határai megmaradnak; a GUI nem válik tool- vagy döntési authority-vé.", "Canonical role boundaries remain intact; the GUI does not become a tool or decision authority."); color: shell.textMuted; wrapMode: Text.WordWrap }
            Rectangle {
                Layout.fillWidth: true; Layout.fillHeight: true; radius: 8; color: shell.surface1
                ScrollView { anchors.fill: parent; anchors.margins: 8; TextArea { readOnly: true; wrapMode: TextEdit.Wrap; text: askDialog.transcript.length ? askDialog.transcript : shell.t("Nincs még üzenet.", "No messages yet.") } }
            }
            TextArea { id: prompt; Layout.fillWidth: true; Layout.preferredHeight: 100; wrapMode: TextEdit.Wrap; placeholderText: shell.t("Írd be a kérdésed…", "Type your question…") }
            RowLayout {
                Layout.fillWidth: true
                Button {
                    text: shell.t("Küldés", "Send"); enabled: prompt.text.trim().length > 0
                    onClicked: {
                        const q=prompt.text.trim(); const prefix=askDialog.transcript.length ? "\n\n" : ""
                ...
                Button { text: shell.t("Beállítások", "Settings"); onClicked: { askDialog.close(); shell.navigateKey("settings") } }
    ...
    header: ToolBar {
        ...
        RowLayout {
            ...
            ToolButton { text: "←"; enabled: shell.navigationHistory.length > 0; onClicked: shell.goBack(); ToolTip.visible: hovered; ToolTip.text: shell.t("Vissza", "Back") }
            ...
            ToolButton { text: "↻"; onClicked: { fa3Repository.refresh(); fa3ResourceTelemetry.refreshNow(); fa3Journal.refresh(400); llmfitClient.refresh() } }
        }
    }
    ...
    Drawer {
        id: remoteDrawer; parent: shell.contentItem; edge: Qt.RightEdge; modal: true; width: Math.min(shell.width*0.78, 1120); height: shell.height
        contentItem: ColumnLayout {
            spacing: 0
            ToolBar { Layout.fillWidth: true; RowLayout { anchors.fill: parent; anchors.leftMargin: 12; anchors.rightMargin: 12; Label { text: "Remote AI Hub"; font.bold: true; Layout.fillWidth: true }; ToolButton { text: "×"; onClicked: remoteDrawer.close() } } }
            RemoteAiHubPanel { Layout.fillWidth: true; Layout.fillHeight: true; textMuted: shell.textMuted; accent: shell.accent; surface1: shell.surface1 }
        }
    }

    Drawer {
        id: logsDrawer; parent: shell.contentItem; edge: Qt.RightEdge; modal: true; width: Math.min(shell.width*0.86, 1320); height: shell.height
        contentItem: ColumnLayout {
            spacing: 0
            ToolBar { Layout.fillWidth: true; RowLayout { anchors.fill: parent; anchors.leftMargin: 12; anchors.rightMargin: 12; Label { text: shell.t("Naplók", "Logs"); font.bold: true; Layout.fillWidth: true }; ToolButton { text: "×"; onClicked: logsDrawer.close() } } }
            LogsPanel { Layout.fillWidth: true; Layout.fillHeight: true; journal: fa3Journal; textPrimary: shell.textPrimary; textMuted: shell.textMuted; surface1: shell.surface1; accent: shell.accent }
        }
    }

    Drawer {
        id: llmfitDrawer; parent: shell.contentItem; edge: Qt.RightEdge; modal: true; width: Math.min(shell.width*0.82, 1200); height: shell.height
        contentItem: ColumnLayout {
            spacing: 0
            ToolBar { Layout.fillWidth: true; RowLayout { anchors.fill: parent; anchors.leftMargin: 12; anchors.rightMargin: 12; Label { text: "Model Manager · llmfit"; font.bold: true; Layout.fillWidth: true }; ToolButton { text: "×"; onClicked: llmfitDrawer.close() } } }
            LlmfitPanel { Layout.fillWidth: true; Layout.fillHeight: true; client: llmfitClient; textMuted: shell.textMuted; accent: shell.accent; surface1: shell.surface1 }
        }
    }
}
