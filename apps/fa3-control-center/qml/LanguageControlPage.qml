...
Item {
    ...
    // Backend-bindable runtime projection. Defaults must never fabricate PASS/NO.
    property var nativeLanguages: []
    property var mediatedLanguages: []
    property string executionLanguage: "UNKNOWN"
    property bool mediationStateKnown: false
    ...
    property string cloudPolicyStatus: "PENDING_BACKEND"
    ...
    // Operator preferences only; canonical runtime policy remains authoritative.
    ...
    function languageList(values) { return values && values.length ? values.join(", ") : "UNKNOWN" }
    function confidenceText() { return confidence < 0 ? "UNKNOWN" : Math.round(confidence * 100) + "%" }
    function mediationText() { return mediationStateKnown ? (mediationRequired ? "YES" : "NO") : "UNKNOWN" }
    function statusColor(status) {
        const value = String(status).toUpperCase()
        if (value === "PASS" || value === "ALLOWED") return "#55a868"
        if (value === "FAIL" || value === "DENIED" || value === "DENIED_BY_CONTRACT") return "#c85c5c"
        return "#d99b32"
    }
    ...
    ScrollView {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                ColumnLayout {
                    Layout.fillWidth: true
                    Label { text: root.t("Nyelvi vezérlés", "Language Control"); font.pixelSize: root.px(24); font.bold: true }
                    Label {
                        Layout.fillWidth: true
                        text: root.t(
                ...
                Label { text: root.profileId + " · P0/MUST"; color: root.accent; font.pixelSize: root.px(10); font.bold: true }
            }
            ...
            Card {
                ...
                RowLayout {
                    ...
                    ComboBox {
                        ...
                        onActivated: { root.viewMode = currentText; root.settings.setValue("languageControl/viewMode", currentText) }
                    }
                    Item { Layout.fillWidth: true }
                    Label { text: "GUI i18n ≠ Language Bridge"; color: root.textMuted; font.pixelSize: root.px(10) }
                }
            }
            ...
            Card {
                ...
                ColumnLayout {
                    ...
                    anchors.margins: 16
                    spacing: 9
                    Label { text: root.t("Felhasználói nyelv", "User language"); font.pixelSize: root.px(16); font.bold: true }
                    RowLayout {
                        Layout.fillWidth: true
                        Label { text: root.t("Bemenet", "Input"); Layout.preferredWidth: Math.round(100 * root.uiScale) }
                        ComboBox {
                            ...
                            onActivated: { root.userLanguage = currentText; root.settings.setValue("languageControl/userLanguage", currentText) }
                            onAccepted: { root.userLanguage = editText; root.settings.setValue("languageControl/userLanguage", editText) }
                        }
                        ...
                        ComboBox {
                            ...
                            onActivated: { root.outputLanguage = currentText; root.settings.setValue("languageControl/outputLanguage", currentText) }
                            onAccepted: { root.outputLanguage = editText; root.settings.setValue("languageControl/outputLanguage", editText) }
                        }
                    }
                    CheckBox {
                        text: root.t("Natív modellnyelv előnyben", "Prefer native model language")
                        checked: root.preferNative
                        onToggled: { root.preferNative = checked; root.settings.setValue("languageControl/preferNative", checked) }
                    }
                    CheckBox {
                        text: root.t("Közvetített modell engedélyezése", "Allow mediated model language")
                        checked: root.allowMediated
                        onToggled: { root.allowMediated = checked; root.settings.setValue("languageControl/allowMediated", checked) }
                    }
                }
            }
            ...
            Card {
                ...
                ColumnLayout {
                    ...
                    anchors.margins: 16
                    spacing: 9
                    Label { text: root.t("Modellnyelvi capability", "Model language capability"); font.pixelSize: root.px(16); font.bold: true }
                    ...
                    FieldRow { fieldLabel: root.t("Közvetítés szükséges", "Mediation required"); fieldValue: root.mediationText() }
            ...
            Card {
                ...
                ColumnLayout {
                    ...
                    anchors.margins: 16
                    spacing: 9
                    Label { text: root.t("Policy & validáció", "Policy & validation"); font.pixelSize: root.px(16); font.bold: true }
                    RowLayout {
                        Layout.fillWidth: true
                    ...
                    CheckBox {
                        ...
                        onToggled: { root.localFirst = checked; root.settings.setValue("languageControl/localFirst", checked) }
                    }
                    CheckBox {
                        text: root.t("Cloud fordítás kérése", "Request cloud translation")
                        checked: root.cloudRequested
                        onToggled: { root.cloudRequested = checked; root.settings.setValue("languageControl/cloudRequested", checked) }
                    }
                    FieldRow {
                        fieldLabel: root.t("Cloud policy", "Cloud policy")
                        fieldValue: root.dataClassification === "SECRET" ? "DENIED_BY_CONTRACT" : root.cloudPolicyStatus
                        valueColor: root.statusColor(fieldValue)
                    }
                    FieldRow {
                        fieldLabel: root.t("Cloud effektív", "Cloud effective")
                        fieldValue: root.dataClassification === "SECRET" ? "NO" : (root.cloudPolicyStatus === "ALLOWED" ? (root.cloudEffective ? "YES" : "NO") : "UNKNOWN")
                    }
                    RowLayout {
                        Layout.fillWidth: true
                        ...
                        ComboBox {
                            ...
                            onActivated: { root.validationMode = currentText; root.settings.setValue("languageControl/validationMode", currentText) }
                        }
                    }
                    FieldRow { fieldLabel: root.t("Validációs státusz", "Validation status"); fieldValue: root.validationStatus; valueColor: root.statusColor(root.validationStatus) }
            ...
            Card {
                ...
                ColumnLayout {
                    ...
                    anchors.margins: 16
                    spacing: 9
                    Label { text: root.t("Provenance & evidence", "Provenance & evidence"); font.pixelSize: root.px(16); font.bold: true }
                    FieldRow { fieldLabel: "Evidence ID"; fieldValue: root.evidenceId }
                    FieldRow { fieldLabel: root.t("Védett tokenek", "Protected tokens"); fieldValue: root.protectedTokenStatus; valueColor: root.statusColor(root.protectedTokenStatus) }
                    FieldRow { fieldLabel: root.t("Beszéd útvonal", "Speech route"); fieldValue: root.speechRoute }
                    Label {
                        Layout.fillWidth: true
                        text: root.t(
                            "Az eredeti input authoritative; a fordítás derived projection. UNKNOWN/PENDING_BACKEND soha nem PASS.",
                            "Original input is authoritative; translation is a derived projection. UNKNOWN/PENDING_BACKEND is never PASS.")
                        color: "#d99b32"
                        wrapMode: Text.WordWrap
                        font.pixelSize: root.px(11)
                    }
            ...
            Label {
                ...
                Layout.rightMargin: 24
                text: root.t(
                    "SECRET adatnál a külső fordítás tiltott; a GUI-kérés ezt nem írhatja felül.",
                    "External translation is forbidden for SECRET data; a GUI request cannot override this rule.")
                color: "#c85c5c"
                wrapMode: Text.WordWrap
                font.pixelSize: root.px(12)
                font.bold: true
