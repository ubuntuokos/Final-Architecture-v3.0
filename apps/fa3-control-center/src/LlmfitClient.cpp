...
QJsonObject candidateProjection(const QVariantMap &model)
{
    QJsonObject candidate;
    const QStringList keys = {
        "name", "provider", "parameter_count", "fit_label", "fit_level", "best_quant",
        "memory_required_gb", "estimated_tps", "runtime", "runtime_label", "estimate_confidence",
        "estimate_confidence_label"
    };
    for (const auto &key : keys) {
        if (model.contains(key)) candidate.insert(key, QJsonValue::fromVariant(model.value(key)));
    }
    return candidate;
}
}
...
QString LlmfitClient::draftRationale(const QString &purpose, const QVariantMap &model,
                                     bool acceleratorRequired, int maxContext) const
{
    QJsonArray resourceClasses;
    resourceClasses.append("CPU");
    resourceClasses.append("MEMORY");
    if (acceleratorRequired) resourceClasses.append("ACCELERATOR");

    QJsonObject envelope;
    envelope.insert("contract_id", "FA3-WORKLOAD-RESOURCE-ENVELOPE-001");
    envelope.insert("requested_resource_classes", resourceClasses);
    envelope.insert("accelerator_required", acceleratorRequired);
    envelope.insert("hrb_authorization_required", true);
    envelope.insert("accelerator_discovery_required", acceleratorRequired);
    envelope.insert("accelerator_lease_required", acceleratorRequired);
    envelope.insert("accelerator_guard_required", acceleratorRequired);
    envelope.insert("context_tokens", qBound(1024, maxContext, 1048576));
    ...
    root.insert("accelerator_guard_profile", acceleratorRequired ? "FA3-ACCEL-GUARD-001" : "NOT_APPLICABLE");
    root.insert("model_routing_authority", "FA3-AUTH-MODEL-ROUTER-001");
    root.insert("evidence_requirement", "MEASURED_RUNTIME_EVIDENCE_REQUIRED");
    root.insert("estimate_is_runtime_evidence", false);
    root.insert("current_host_runtime_promotion_claim", false);

    return QString::fromUtf8(QJsonDocument(root).toJson(QJsonDocument::Compact));
}

QString LlmfitClient::benchmarkDraftRationale(const QVariantMap &model, bool acceleratorRequired, int maxContext) const
{
    return draftRationale(QStringLiteral("VALIDATE_LLMFIT_ESTIMATE_WITH_MEASURED_RUNTIME_BENCHMARK"),
                          model, acceleratorRequired, maxContext);
}

QString LlmfitClient::placementDraftRationale(const QVariantMap &model, bool acceleratorRequired, int maxContext) const
{
    return draftRationale(QStringLiteral("REQUEST_HRB_MEDIATED_RUNTIME_PLACEMENT_REVIEW"),
                          model, acceleratorRequired, maxContext);
}
