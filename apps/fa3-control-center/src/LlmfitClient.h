...
class LlmfitClient final : public QObject
{
    ...
    Q_PROPERTY(QString observationAuthority READ observationAuthority CONSTANT)
...
public:
    ...
    QString providerId() const { return QStringLiteral("FA3-PROVIDER-LLMFIT-001"); }
    QString observationAuthority() const { return QStringLiteral("OBSERVED_NON_AUTHORITATIVE"); }
    ...
    Q_INVOKABLE QString benchmarkDraftRationale(const QVariantMap &model, bool acceleratorRequired, int maxContext) const;
    Q_INVOKABLE QString placementDraftRationale(const QVariantMap &model, bool acceleratorRequired, int maxContext) const;
...
private:
    ...
    QString draftRationale(const QString &purpose, const QVariantMap &model, bool acceleratorRequired, int maxContext) const;
