...
def _report(
    *,
    ...
    return {
        "schema": "fa3.host-bootstrap-admission-orchestrator-report.v1",
...
def doctor(root: Path, runner: Runner = run_command) -> tuple[int, dict[str, Any]]:
    ...
    if checks["acquire_client"]:
        checks["acquire_bridge_doctor"] = runner([str(ACQUIRE_CLIENT), "--doctor"], 15).returncode == 0
    else:
        checks["acquire_bridge_doctor"] = False
...
def main() -> int:
    ...
    if args.command == "doctor":
        ...
        env = dict(os.environ)
        env.setdefault(
            "FA3_HRB_ACQUIRE_COMMAND",
            "/usr/local/bin/fa3-host-resource-broker-acquire --workload {workload} --lease-output {lease} --accelerator-uuid {gpu_uuid}",
        )
        try:
            proc = subprocess.run(
                [str(smoke), "smoke"],
