...
def gate(root:Path)->dict[str,Any]:
    ...
    if not findings:
        ...
        if not (isinstance(decision.get("baseline",{}).get("capability_count_after"),int) and decision.get("baseline",{}).get("capability_count_after")<=capability_count and decision.get("baseline",{}).get("new_architectural_authorities")==0 and decision.get("closure_semantics",{}).get("current_host_surfaces")=="PENDING_CURRENT_HOST" and decision.get("closure_semantics",{}).get("global_promotion_claim") is False): findings.append(_finding("MODERN-007","Decision overclaims closure or changes authority/capability baseline"))
