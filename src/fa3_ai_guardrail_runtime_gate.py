...
def evaluate(root: Path) -> dict[str, Any]:
    ...
    checks = [
        check("capability-count-stable", all(x.get("capability_count", CAPABILITY_COUNT) == CAPABILITY_COUNT for x in [profile, contract, provider, hrb, systemd]), "capability count follows active release baseline"),
        ...
        check("decision-closed", decision.get("id") == DECISION_ID and decision.get("status") == "CANONICAL_CLOSED" and decision.get("new_capabilities") == 0 and decision.get("new_architectural_authorities") == 0 and isinstance(decision.get("capability_count_after"), int) and decision.get("capability_count_after") <= CAPABILITY_COUNT and decision.get("mandatory_canonical_rules") == P0_INVARIANTS, "decision is closed with no capability/authority change"),
