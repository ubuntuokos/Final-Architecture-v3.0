...
def capability_task_valid(*, capability: str, task: str, validated_mapping: dict[str, list[str]], verdict: str) -> bool:
    tasks = validated_mapping.get(capability, [])
...
def unit_projection_valid(unit_text: str) -> tuple[bool, list[str]]:
    """Fail closed on the failure modes observed in the current-host PoC.

    This is deliberately a conservative semantic lint, not a complete systemd parser.
    Host-version feature validation still belongs to deployment/runtime admission.
    """
    findings: list[str] = []
    section = ""
...
def _forbidden_hardcoded_runtime_value(obj: Any, path: tuple[str, ...] = ()) -> bool:
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            if key.lower() in forbidden_numeric_keys and isinstance(value, (int, float)):
                return True
            if _forbidden_hardcoded_runtime_value(value, path + (key,)):
                return True
    elif isinstance(obj, list):
        return any(_forbidden_hardcoded_runtime_value(v, path) for v in obj)
    return False
...
def evaluate(root: Path) -> dict[str, Any]:
    ...
    if missing:
        return {
            ...
            "gate_id": GATE_ID,
            "status": "FAIL",
            "checks": [check("required-artifacts", False, f"missing: {', '.join(missing)}")],
        }
