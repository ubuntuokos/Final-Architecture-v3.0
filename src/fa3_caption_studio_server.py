...
def _html(mode:str)->str:
    ...
    extra=('''<div class="row"><label>Voice identity <input id="voice" value="FA3-VOICE-DEFAULT"></label><label>Mode <select id="nmode"><option>NARRATION</option><option>VOICE_OVER</option><option>DUBBING</option><option>AUDIO_DESCRIPTION_READY</option></select></label><button onclick="narrate()">Build narration plan</button></div>''' if narration else '''<div class="row"><button onclick="exportFmt('srt')">Export SRT</button><button onclick="exportFmt('vtt')">Export VTT</button><button onclick="exportFmt('ass')">Export ASS</button><button onclick="exportFmt('ttml')">Export TTML</button><button onclick="exportFmt('json')">Export FA3 JSON</button></div>''')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self)->None:
        ...
        try:
            ...
            elif path=="/api/narration-plan": self._json(200,build_narration_plan(payload["document"],voice_identity_ref=payload.get("voice_identity_ref","FA3-VOICE-DEFAULT"),narration_mode=payload.get("mode","NARRATION")))
