...
def gate(root: Path) -> dict[str, Any]:
    ...
    if missing:
        return {"schema":"fa3.pytorch3d-gate-report.v1","gate_set_id":GATESET_ID,"result":"FAIL","blocking_findings":len(missing),"findings":[finding("P3D-000", f"missing {path}") for path in missing]}
    root_profile=load(root,"root_profile"); profile=load(root,"profile"); geometry_contract=load(root,"geometry_contract"); contract=load(root,"contract"); provider=load(root,"provider"); decision=load(root,"decision"); reference=load(root,"reference"); runtime=load(root,"runtime"); gate_record=load(root,"gate"); enforcement=load(root,"enforcement"); release=load(root,"release"); evidence=load(root,"evidence"); registry=load(root,"registry"); global_policy=load(root,"global_policy"); global_release=load(root,"global_release")
    records=registry.get("records",[]); cap032=next((item for item in records if item.get("subject_id")==CAPABILITY_ID),{}); projection=cap032.get("pytorch3d_provider_projection_status",{}); global_reconciliation=global_release.get("pytorch3d_reconciliation",{}); manifest_paths={item.get("path") for item in global_release.get("manifest",[])}; inventory=global_release.get("overlay_inventory",{}); authorities=provider.get("authority_boundaries",{}); policy=reference_policy_conformance()
    checks = [
        ...
        ("P3D-003", profile.get("capability_bindings")==[CAPABILITY_ID] and profile.get("capability_count")==CAPABILITY_COUNT and profile.get("new_capability") is False and profile.get("new_architectural_authority") is False, "CAP-032/no-new-capability invariant failed against active release baseline"),
        ...
        ("P3D-019", decision.get("status")=="CANONICAL_CLOSED" and decision.get("profile_id")==PROFILE_ID and decision.get("provider_id")==PROVIDER_ID and decision.get("baseline_effect",{}).get("capability_count_after")==CAPABILITY_COUNT and decision.get("baseline_effect",{}).get("new_architectural_authorities")==0, "decision closure/baseline invariant failed against active release baseline"),
        ...
        ("P3D-031", global_reconciliation.get("provider_id")==PROVIDER_ID and global_reconciliation.get("reconciliation_status")=="GLOBAL_RELEASE_INVENTORY_EVIDENCE_RECONCILED_REFERENCE_PASS_CURRENT_HOST_PENDING" and global_reconciliation.get("runtime_activation_status")=="NOT_PRODUCTION_PROMOTED" and global_reconciliation.get("capability_count_after")==CAPABILITY_COUNT and PATHS["provider"] in inventory.get("provider_records",[]) and PATHS["provider"] in manifest_paths and PATHS["evidence"] in manifest_paths, "global release/inventory/evidence reconciliation missing or active baseline drifted"),
        ("P3D-032", root_profile.get("id")=="FA3-3D-GEOM-001" and root_profile.get("canonical_root") is True and root_profile.get("capability_count")==CAPABILITY_COUNT and geometry_contract.get("status")=="CANONICAL", "sole geometry root invariant changed or active baseline drifted"),
    ]
    findings=[finding(code,message) for code,ok,message in checks if not ok]
    report={"schema":"fa3.pytorch3d-gate-report.v1","gate_set_id":GATESET_ID,"result":"PASS" if not findings else "FAIL","blocking_findings":len(findings),"findings":findings,"rules_checked":EXPECTED_RULE_COUNT,"active_release_capability_count":CAPABILITY_COUNT,"provider_executable_conformance":policy,"current_host_runtime_promoted":False}
    out=root/"reports/pytorch3d-gate-report.json"; out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,indent=2)+"\n",encoding="utf-8")
    return report
