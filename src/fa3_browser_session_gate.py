...
def gate(root:Path)->dict[str,Any]:
    ...
    required=["canonical/profiles/FA3-BROWSER-SESSION-INTERACTION-001.json","canonical/contracts/FA3-BROWSER-SESSION-INTERACTION-CONTRACTS-001.json","canonical/intents/FA3-BROWSER-SESSION-INTERACTION-APPLICATION-INTENT-001.json","canonical/assessments/FA3-BROWSER-SESSION-INTERACTION-REUSE-ASSESSMENT-001.json","canonical/providers/FA3-PROVIDER-BROWSER-SESSION-BRIDGE-001.json","canonical/browser-session-current-host-enforcement.json","canonical/FA3-GATE-BROWSER-SESSION-CURRENT-HOST-001.json","canonical/decisions/FA3-DEC-HARDWARE-SAFETY-2026-09-26.json","browser/fa3-session-extension/manifest.json","browser/fa3-session-extension/service_worker.js","libexec/fa3-browser-session-native-host.py","src/fa3_browser_session_bridge.py","src/fa3_browser_session_provider.py","src/fa3_browser_session_current_host_gate.py",".github/workflows/fa3-browser-session-current-host.yml","tests/test_browser_session_bridge.py","tests/test_browser_session_gate.py","docs/browser-session-interaction.md"]+[f"canonical/actions/{a}.json" for a in sorted(ACTION_IDS)]
    for rel in required:
    ...
    if p.get("capability_count")!=CAPABILITY_COUNT or p.get("new_capability") is not False or p.get("new_architectural_authority") is not False:findings.append({"code":"BSI-003","message":"capability/authority invariant broken"})
    ...
    hs=p.get("hardware_safety_envelope",{})
    ...
    phs=provider.get("hardware_safety_envelope",{})
    ...
    if cur.get("hardware_parameter_mutation")!="DENY" or cur.get("hardware_safety_protection_bypass")!="DENY" or cur.get("hardware_safety_decision_id")!="FA3-DEC-HARDWARE-SAFETY-2026-09-26":findings.append({"code":"BSI-024A","message":"current-host hardware safety invariant broken"})
    if cur.get("cli_extension_test_browser_selection")!="CAPABILITY_AWARE_FAIL_CLOSED" or cur.get("automatic_browser_install_or_replacement")!="DENY":findings.append({"code":"BSI-024B","message":"extension-test browser selection/coexistence invariant broken"})
    ...
    return{"schema":"fa3.browser-session-gate-report.v1","gate_id":GATE_ID,"result":"PASS" if not findings else"FAIL","findings":findings,"capability_count":CAPABILITY_COUNT,"capability_delta":0,"authority_delta":0,"current_host_runtime_claim":False,"human_completion_claim":False,"global_promotion_claim":False}
def main()->int:
