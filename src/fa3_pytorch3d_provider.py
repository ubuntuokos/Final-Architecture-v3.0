...
def validate_build_candidate(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = [
        ...
        _check("hardware-derived-targets", bool(candidate.get("target_accelerator_architectures")) and candidate.get("target_architectures_source") == "CURRENT_HOST_ADMISSION_HRB_BOUND_COMPUTE_PROFILE", "target architectures derive from admitted hardware"),
        _check("admission-build-budget", candidate.get("build_parallelism_source") == "CURRENT_HOST_ADMISSION_COMPUTE_PROFILE", "build parallelism derives from the validated current-host compute profile"),
