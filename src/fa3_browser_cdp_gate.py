...
def gate(root):
    root=root.resolve();f=[];required=["canonical/providers/FA3-PROVIDER-BROWSER-CDP-001.json","canonical/FA3-GATE-BROWSER-CDP-PROVIDER-001.json","canonical/browser-cdp-provider-enforcement.json","canonical/browser-cdp-current-host-enforcement.json","canonical/assessments/FA3-BROWSER-CDP-PROVIDER-DECISION-ASSESSMENT-2026-09-23.json","src/fa3_browser_cdp_provider.py","src/fa3_browser_cdp_current_host_gate.py","tests/test_browser_cdp_provider.py",".github/workflows/fa3-browser-cdp-provider.yml",".github/workflows/fa3-browser-cdp-current-host.yml","docs/browser-cdp-provider.md"]
    for rel in required:
    ...
    if p.get("new_capability") is not False or p.get("new_architectural_authority") is not False or p.get("capability_count")!=143:f.append({"code":"BCD-003"})
    ...
    return{"schema":"fa3.browser-cdp-provider-gate-report.v1","gate_id":GATE_ID,"provider_id":PROVIDER_ID,"result":"PASS" if not f else "FAIL","findings":f,"capability_count":143,"capability_delta":0,"authority_delta":0,"global_promotion_claim":False}
def main():
