...
def gate(root: Path) -> dict[str, Any]:
    ...
    checks: list[tuple[str, bool, str, dict[str, Any]]] = [
        ...
        ("DEHC-006", not hardcodes, "active executable Python still contains forbidden 143 count hardcodes", {"hardcodes": hardcodes[:50]}),
        ...
        ("DEHC-008", "range(1,CAPS+1)" in module_text["src/fa3_enforce.py"].replace(" ", "") and "CAPS=143" not in module_text["src/fa3_enforce.py"].replace(" ", ""), "global enforcement still derives catalog cardinality from a literal", {}),
        ("DEHC-009", "capability_count_after\": capability_count" in module_text["scripts/fa3_reconcile_release_projection.py"] or '"capability_count_after": capability_count' in module_text["scripts/fa3_reconcile_release_projection.py"], "release reconciler still writes a literal capability count", {}),
