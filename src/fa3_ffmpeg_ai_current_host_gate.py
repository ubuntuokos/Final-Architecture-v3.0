...
from fa3_ffmpeg_ai_current_host import (
    ...
    CURRENT_HOST_EXECUTABLE_GATE_ID,
...
def gate(root: Path, receipt_path: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if receipt.get("fixture_semantics") == "SYNTHETIC_REFERENCE_FIXTURE_NOT_CURRENT_HOST":
            findings.append({
                "code": "FFMPEG-AI-HOST-015",
                "severity": "P0",
                "message": "synthetic/unit-test fixture cannot be presented as production current-host evidence",
            })
    except Exception as exc:
        receipt = {}
        findings = [{
            "code": "FFMPEG-AI-HOST-000",
            "severity": "P0",
            "message": "current-host receipt missing or unreadable",
            "error": repr(exc),
        }]
    ...
    report = {
        ...
        "hardware_semantics": "LIVE_FACTS_EVIDENCE_ONLY_HRB_IS_ADMISSION_AND_PLACEMENT_AUTHORITY",
    }
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Validate FA3 FFmpeg neural-media production current-host evidence")
