...
def evaluate(root: Path) -> dict[str, Any]:
    ...
    smt_plan = build_thread_plan(
        topology,
        _request(requested_threads=32, allow_smt=True, benchmark_evidence=True, explicit_smt_admission=True),
    )
    ...
    checks = [
        ...
        check("synthetic-portability-fixture", host_plan["visible_physical_cores"] == 16 and host_plan["visible_logical_cpus"] == 32, "generic synthetic fixture exercises two NUMA domains without host identity"),
        ...
        check("live-discovery-contract", "TOPOLOGY_MUST_BE_LIVE_DISCOVERED_AND_VALIDATED_AT_BOOT_OR_ADMISSION" in invariants, "boot/admission topology validation is mandatory"),
        check("global-nproc-fanout-forbidden", all(token in forbidden for token in ["nproc", "fixed omp thread count", "fixed numexpr thread count", "environment variables"]), "global nproc and fixed thread fan-out are forbidden"),
        check("physical-core-first-hostwide", host_plan["thread_budget"] == 16 and not host_plan["uses_smt_above_physical_budget"], "host-wide default stops at physical cores"),
        check("cpuset-numa-aware", node_plan["visible_physical_cores"] == 8 and node_plan["thread_budget"] == 8 and node_plan["numa_node"] == 0, "NUMA-local admitted cpuset derives its budget from visible physical cores"),
        check("smt-denied-without-evidence", denied(lambda: build_thread_plan(topology, _request(requested_threads=32))), "SMT request above physical-core budget fails closed without evidence"),
        check("smt-admitted-with-evidence", smt_plan["thread_budget"] == 32 and smt_plan["uses_smt_above_physical_budget"], "SMT can be admitted only with benchmark and explicit admission"),
