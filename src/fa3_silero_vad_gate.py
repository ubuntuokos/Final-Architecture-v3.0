...
BOOTSTRAP_IDENTITY = ROOT / "evidence/reference/FA3-SILERO-VAD-MODEL-BOOTSTRAP-IDENTITY-001.json"
...
REQUIRED_FILES = [
    PROFILE, CONTRACTS, PROVIDER, ALLOWLIST, DECISION, REFERENCE, BOOTSTRAP_IDENTITY,
...
def run_gate() -> tuple[bool, list[str]]:
    ...
    bootstrap_identity = load(BOOTSTRAP_IDENTITY)
    ...
    require("ALLOWLIST_PATH" in adapter_text and "validate_model_identity" in adapter_text, "adapter does not enforce canonical model identity", errors)
    ...
    require('"production_promotion_eligible": False' in promotion_text, "current-host collector may self-promote production", errors)
    ...
    require(allowlist.get("bootstrap_identity_receipt") == bootstrap_identity.get("id"), "bootstrap identity receipt not bound to allowlist", errors)
    require(bootstrap_identity.get("status") == "PASS", "bootstrap identity receipt must pass", errors)
    require(bootstrap_identity.get("production_admitted") is False, "bootstrap identity may not promote production", errors)
    artifacts = allowlist.get("artifacts", [])
    require(len(artifacts) >= 3, "expected Silero model candidates missing", errors)
    ...
    require(all(valid_sha256(a.get("sha256")) for a in artifacts), "all Silero artifacts require canonical SHA-256", errors)
    require(all(a.get("identity_admitted") is True for a in artifacts), "all listed Silero artifact identities must be admitted", errors)
    require(all(a.get("production_admitted") is False for a in artifacts), "artifact promoted without current-host evidence", errors)
    bootstrap_by_name = {a["name"]: a for a in bootstrap_identity.get("artifacts", [])}
    for artifact in artifacts:
        evidence = bootstrap_by_name.get(artifact.get("name"), {})
        require(evidence.get("sha256") == artifact.get("sha256"), f"bootstrap SHA-256 mismatch: {artifact.get('name')}", errors)
        require(evidence.get("size_bytes") == artifact.get("size_bytes"), f"bootstrap size mismatch: {artifact.get('name')}", errors)
    ...
    require(len(enforcement.get("rules", [])) >= 32, "enforcement rule set unexpectedly reduced", errors)
