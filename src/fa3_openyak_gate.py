...
RULES = [
    ...
    "OPENYAK_WAYLAND_PREFERRED_X11_SUPPORTED_CURRENT_HOST_SMOKE_REQUIRED",
...
def desktop_runtime_valid(*, display_protocol: str, forced_x11: bool, wayland_smoke: bool, direct_gpu_allocation: bool) -> bool:
    session_supported = display_protocol in {"wayland", "x11"}
    session_smoke = wayland_smoke if display_protocol == "wayland" else True
    return session_supported and not forced_x11 and session_smoke and not direct_gpu_allocation
...
def regression_cases() -> list[dict[str, Any]]:
    ...
    add(22, "Wayland is preferred and both Wayland/X11 require session-native current-host smoke", desktop_runtime_valid(display_protocol="wayland", forced_x11=False, wayland_smoke=True, direct_gpu_allocation=False) and desktop_runtime_valid(display_protocol="x11", forced_x11=False, wayland_smoke=False, direct_gpu_allocation=False), not desktop_runtime_valid(display_protocol="unknown", forced_x11=False, wayland_smoke=False, direct_gpu_allocation=False))
