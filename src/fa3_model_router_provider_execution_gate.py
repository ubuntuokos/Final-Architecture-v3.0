...
def gate(root: Path) -> dict[str, Any]:
    ...
    if not producer.is_file() or not config_schema.is_file() or not provisioner.is_file(): f.append(finding("PEX-CH-002","current-host producer/config/provisioning contract missing"))
        ...
        if "terminal_billing_blocker" not in producer_text or "credit_balance_exhausted" not in producer_text or "project_spend_limit_exceeded" not in producer_text:
            f.append(finding("PEX-CH-011","terminal upstream billing/quota blockers must stop model probing fail-closed"))
