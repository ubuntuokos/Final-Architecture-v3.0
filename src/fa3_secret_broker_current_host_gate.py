...
def validate(x:dict)->list[str]:
    ...
    checks=x.get("checks",{})
    for k in ["authorized_single_secret_get","systemd_loadcredential_projection_pass","unauthorized_consumer_denied","raw_vault_access_denied","bulk_export_absent","credential_scope_enforced","audit_contains_no_raw_secret","secret_absent_from_argv","secret_absent_from_environment","broker_health_pass","explicit_unmount_pass","luks_close_pass","fa3_exit_closed_state_pass","opaque_backup_copy_pass","restore_unlock_pass","restore_mount_pass","restore_broker_health_pass"]:
