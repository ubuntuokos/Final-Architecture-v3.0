...
from fa3_lut_color_gate import gate as lut_color_gate
...
def static_check(root:Path):
    ...
    if "FA3-LYNXHUB-GATESET-001" not in pol.get("mandatory_reference_gates",[]):
    ...
    if "FA3-GPU-KERNEL-RUNTIME-GATESET-001" not in pol.get("mandatory_reference_gates",[]):
        fs.append(finding("FA3-STATIC-076","GPU kernel runtime/autotuning gate is not bound into global enforcement policy"))
    ...
    if "FA3-LUT-COLOR-GATESET-001" not in pol.get("mandatory_reference_gates",[]):
        fs.append(finding("FA3-STATIC-099","LUT registration and color-transform gate is not bound into global enforcement policy"))
    ...
    gpu_kernel_runtime_ref=gpu_kernel_runtime_gate(root)
    if gpu_kernel_runtime_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-077","GPU kernel runtime/autotuning/provider eligibility regression gate failed",gpu_kernel_runtime_gate=gpu_kernel_runtime_ref))
    ...
    video_provider_lifecycle_ref=video_provider_lifecycle_gate(root)
    if video_provider_lifecycle_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-079","Video provider lifecycle/backend/cache regression gate failed",video_provider_lifecycle_gate=video_provider_lifecycle_ref))
    ...
    lut_color_ref=lut_color_gate(root)
    if lut_color_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-100","LUT registration and color-transform canonical gate failed",lut_color_gate=lut_color_ref))
...
def main():
    ...
    try:
        ...
        if a.command=="gpu-kernel-runtime":
            x=gpu_kernel_runtime_gate(root); print(json.dumps(x,indent=2)); return OK if x["result"]=="PASS" else BLOCKED
        if a.command=="gpu-kernel-runtime-current-host":
            x=gpu_kernel_runtime_current_host_gate(root); print(json.dumps(x,indent=2)); return OK if x["result"]=="PASS" else BLOCKED
        ...
        if a.command=="video-provider-lifecycle":
            x=video_provider_lifecycle_gate(root); print(json.dumps(x,indent=2)); return OK if x["result"]=="PASS" else BLOCKED
        ...
        if a.command=="openfx-interop":
            x=openfx_interop_gate(root); print(json.dumps(x,indent=2)); return OK if x["result"]=="PASS" else BLOCKED
        if a.command=="lut-color":
            x=lut_color_gate(root); print(json.dumps(x,indent=2)); return OK if x["result"]=="PASS" else BLOCKED
