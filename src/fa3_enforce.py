...
def static_check(root:Path):
    ...
    if model_router_provider_execution_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-128","Model Router Provider Execution gate failed",model_router_provider_execution_gate=model_router_provider_execution_ref))
    ...
    if "FA3-MODEL-ROUTER-PROVIDER-EXECUTION-GATESET-001" not in pol.get("mandatory_reference_gates",[]):
        fs.append(finding("FA3-STATIC-128","Model Router Provider Execution gate is not bound into global enforcement policy"))
