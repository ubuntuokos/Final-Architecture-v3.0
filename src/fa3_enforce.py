...
from fa3_ai_security_testing_fabric_gate import gate as ai_security_testing_fabric_gate
...
def static_check(root:Path):
    ...
    ai_security_testing_fabric_ref=ai_security_testing_fabric_gate(root)
    if ai_security_testing_fabric_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-136","AI Security Testing Fabric boundary/removal gate failed",ai_security_testing_fabric_gate=ai_security_testing_fabric_ref))
...
def main():
    ...
    try:
        ...
        if a.command=="ai-security-testing-fabric":
            x=ai_security_testing_fabric_gate(root); print(json.dumps(x,indent=2,ensure_ascii=False)); return OK if x["result"]=="PASS" else BLOCKED
