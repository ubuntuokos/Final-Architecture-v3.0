...
from fa3_runtime_hardening_current_host_gate import gate as runtime_hardening_current_host_gate
...
def static_check(root:Path):
    ...
    if "FA3-RUNTIME-HARDENING-GATESET-001" not in pol.get("mandatory_reference_gates",[]):
        fs.append(finding("FA3-STATIC-108","Runtime Hardening mandatory cross-cutting gate is not bound into global enforcement policy"))
    ...
    runtime_hardening_ref=runtime_hardening_gate(root)
    if runtime_hardening_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-109","Runtime Hardening mandatory canonical/current-host-closure regression gate failed",runtime_hardening_gate=runtime_hardening_ref))
...
def main():
    ...
    try:
        ...
        if a.command=="runtime-hardening-current-host":
            x=runtime_hardening_current_host_gate(root); print(json.dumps(x,indent=2)); return OK if x["result"]=="PASS" else BLOCKED
