...
def static_check(root:Path):
    ...
    if "FA3-COORDINATION-GATESET-001" not in pol.get("mandatory_reference_gates",[]):
        fs.append(finding("FA3-STATIC-137","Objective Coordination mandatory zero-authority gate is not bound into global enforcement policy"))
    ...
    if coordination_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-138","Objective Coordination zero-authority regression gate failed",coordination_gate=coordination_ref))
