...
def static_check(root:Path):
    ...
    if gui_current_host_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-128","GUI current-host closure materialization gate failed",gui_current_host_gate=gui_current_host_ref))
    ...
    if "FA3-GUI-CURRENT-HOST-GATESET-001" not in pol.get("mandatory_reference_gates",[]):
        fs.append(finding("FA3-STATIC-128","GUI current-host closure gate is not bound into global enforcement policy"))
