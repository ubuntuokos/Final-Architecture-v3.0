...
from fa3_animation_production_gate import gate as animation_production_gate
...
def static_check(root:Path):
    ...
    animation_production_ref=animation_production_gate(root)
    if animation_production_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-071","Animation production profile/hardware/license regression gate failed",animation_production_gate=animation_production_ref))
...
def main():
    ...
    ap.add_argument("command",choices=("static","release-projection","runtime","terax","kaneo","kanboard","buzz","xcmd","ai-engineering","external-api-discovery","autogpt","ai-infra-guard","ai-infra-guard-current-host","munder-difflin","munder-difflin-executable","muse-code","openhands","stability-sgm","stability-portfolio","developer-agent-coordination","codex","codex-current-host","modular","inference-portability","model-manager","model-manager-current-host","modular-provider","modular-current-host","demucs","demucs-provider","demucs-current-host","acestep","kdenlive-editorial","opencut","hybrid-editorial","animation-production","marketing","blackhole-kdenlive","whisper-stt","whisper-stt-provider","cosyvoice","cosyvoice-current-host","voice-synthesis","cpu-numa-threading","cpu-numa-threading-current-host","mentor","presenton","presenton-current-host","acceptance","promote","all","status"))
    a=ap.parse_args()
    ...
    try:
        ...
        if a.command=="animation-production":
            x=animation_production_gate(root); print(json.dumps(x,indent=2)); return OK if x["result"]=="PASS" else BLOCKED
