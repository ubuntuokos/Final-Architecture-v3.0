...
def static_check(root:Path):
    ...
    if "FA3-AGENCY-AGENTS-GATESET-001" not in pol.get("mandatory_reference_gates",[]):
        fs.append(finding("FA3-STATIC-122","Agency Agents reference-provider gate is not bound into global enforcement policy"))
    ...
    if agency_agents_ref["result"]!="PASS":
        fs.append(finding("FA3-STATIC-122","Agency Agents reference-provider normalization gate failed",agency_agents_gate=agency_agents_ref))
