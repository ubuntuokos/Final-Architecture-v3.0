...
CONTRACT_ID = "FA3-COACH-CONTRACTS-001"
PROVIDER_ID = "FA3-PROVIDER-COACH-LOCAL-001"
DECISION_ID = "FA3-DEC-COACH-2026-09-12"
GATE_ID = "FA3-GATE-COACH-001"
GATESET_ID = "FA3-COACH-GATESET-001"
CONFORMANCE_ID = "FA3-COACH-CONFORMANCE-MATRIX-001"
RUNTIME_ID = "FA3-COACH-RUNTIME-CONFORMANCE-001"
CAPABILITY_COUNT = 143
POLICY = "canonical/enforcement-policy.json"
RELEASE = common.RELEASE
REFERENCE_EVIDENCE = "evidence/reference/fa3-coach-reference-pending.json"
...
def patch_policy() -> None:
    ...
    if GATESET_ID not in gates:
        try:
            mentor_index = gates.index("FA3-MENTOR-GATESET-001")
        except ValueError:
            gates.append(GATESET_ID)
        else:
            gates.insert(mentor_index + 1, GATESET_ID)
...
def patch_release_semantics() -> None:
    ...
    release["coach_reconciliation"] = {
        ...
        "classification": "P0_MUST_PROVIDER_NEUTRAL_GOAL_EXECUTION_FEEDBACK_ACCOUNTABILITY_OVERLAY",
    ...
    note = (
        "FA3-COACH-001 is globally reconciled as a P0/MUST provider-neutral coaching overlay. "
        "User-owned goals, explicit commitment, proposal-only operation, typed action delegation, "
        "FA3-MENTOR-001 knowledge-gap delegation and current-host non-promotion remain mandatory; "
...
def projection_only(snapshot_head: str) -> None:
    ...
    verification["coach_reconciliation_generator"] = "tools/fa3_coach_global_reconcile.py"
    verification["coach_deterministic_regeneration_pass"] = True
    verification["coach_snapshot_head"] = snapshot
    write(RELEASE, release)

    dirty_after = common.dirty_release_surface_paths()
    if dirty_after != [RELEASE]:
        raise RuntimeError(
            "Coach projection-only mode may modify exactly the unified release projection; observed: "
            + ", ".join(dirty_after)
        )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="FA3 Coach deterministic global reconciliation")
