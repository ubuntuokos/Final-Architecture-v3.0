...
def patch_root_profile() -> None:
    ...
    profile["gui_surface"] = {"application": "FA3 Control Center", "menu_label": "FA3 OS", "page_index": 24, "timeline_authority": "FA3-JOURNAL-001"}
...
def patch_event_privacy_gate() -> None:
    ...
    if import_line not in text:
        ...
        if anchor not in text: raise RuntimeError("event/privacy gate import anchor missing")
...
def patch_control_center_cmake() -> None:
    ...
    if prop not in text:
        ...
        if anchor not in text: raise RuntimeError("CMake source property anchor missing")
    ...
    if qml_line not in text:
        ...
        if anchor not in text: raise RuntimeError("CMake QML_FILES anchor missing")
...
def patch_control_center_main() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    search_entry = '        {title: "FA3 OS", detail: "Aktivitás, workstream, privacy és provenance kontextus", category: "FUNCTION", pageIndex: 24},\n'
    if search_entry not in text:
        ...
        if anchor not in text: raise RuntimeError("Main.qml search-index anchor missing")
        text = text.replace(anchor, anchor + search_entry, 1)
    nav = '                        NavButton { iconText: "◎"; label: "FA3 OS"; pageIndex: 24 }\n'
    if nav not in text:
        anchor = '                        NavButton { iconText: "≡"; label: "Napló / Journal"; pageIndex: 12 }\n'
        if anchor not in text: raise RuntimeError("Main.qml navigation anchor missing")
...
'''
    if page_mount not in text:
        anchor = '''                LanguageControlPage {
            ...
            Rectangle {
'''
        replacement = '''                LanguageControlPage {
                    preferences: fa3Preferences
                    panel: window.panel
                    panelRaised: window.panelRaised
                    border: window.border
...
''' + page_mount + '''            }
            ...
            Rectangle {
'''
        if anchor not in text: raise RuntimeError("Main.qml page-mount anchor missing")
...
def patch_reconciliation_tool() -> None:
    ...
    if 'RUNTIME_PROFILE_ID = "FA3-OS-RUNTIME-001"' not in text:
        ...
        if anchor not in text: raise RuntimeError("reconciliation constants anchor missing")
    ...
    if 'policy["fa3_os_runtime_profile_id"]' not in text:
        ...
        if anchor not in text: raise RuntimeError("reconciliation policy anchor missing")
    ...
    if '"runtime_profile_id": RUNTIME_PROFILE_ID' not in text:
        ...
        if anchor not in text: raise RuntimeError("reconciliation release anchor missing")
...
def main() -> int:
    patch_root_profile(); patch_enforcement(); patch_event_privacy_gate(); patch_control_center_cmake(); patch_control_center_main(); patch_reconciliation_tool(); ensure_executable()
    print(json.dumps({"result": "PASS", "materialization": "FA3-OS-RUNTIME-001", "gui_menu": "FA3 OS", "page_index": 24, "ledger_authority": "FA3-JOURNAL-001", "current_host_status": "PENDING_CURRENT_HOST", "capability_count": 143, "new_architectural_authorities": 0}, indent=2))
    return 0
