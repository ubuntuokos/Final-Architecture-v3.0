...
if r.get("schema")!="fa3.secret-broker-current-host-receipt.v1" or r.get("status")!="PASS" or r.get("secret_values_collected") is not False:
    raise SystemExit("Secret Broker current-host receipt is not a valid PASS")
PY
