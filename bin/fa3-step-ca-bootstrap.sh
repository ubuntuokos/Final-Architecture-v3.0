...
fetch_verify(){ local repo="$1" ver="$2" asset="$3" expected="$4" bundle="$5" csum="$6" out="$7" base="https://github.com/$repo/releases/download/v$ver"; mkdir -p "$out"; curl -fL --retry 3 "$base/checksums.txt" -o "$out/checksums.txt"; curl -fL --retry 3 "$base/$asset" -o "$out/$asset"; curl -fL --retry 3 "$base/$asset.sigstore.json" -o "$out/$asset.sigstore.json"; [[ "$(sha "$out/checksums.txt")" == "$csum" ]]; [[ "$(sha "$out/$asset")" == "$expected" ]]; [[ "$(sha "$out/$asset.sigstore.json")" == "$bundle" ]]; (cd "$out" && grep -F "$asset" checksums.txt | sha256sum -c -); "$(cosign_bin)" verify-blob --bundle "$out/$asset.sigstore.json" --certificate-identity-regexp "$IDENTITY_RE" --certificate-oidc-issuer "$ISSUER" "$out/$asset" >/dev/null; }
